...
class BerserkBand : public Item {
    public:
        ...
        void aplicar_efeito(Pokemon &pokemon) override {
            if (pokemon.get_estatistica(0) <= pokemon.get_estatistica_calculos(0) / 2) {
...
public:
    EndureScarf() : Item("Endure Scarf", "Aumenta a defesa do Pokémon quando a vida estiver muito baixa (25% de vida)") {}

    void aplicar_efeito(Pokemon &pokemon) override {
        if (pokemon.get_estatistica(0) < pokemon.get_estatistica_calculos(0) / 4) {
...
class OranBerry : public Item {
    public:
        OranBerry() : Item("Oran Berry", "Recupera 20 de vida ao ser consumida (50% da vida)") {}
